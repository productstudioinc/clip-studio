...
export const TweetActions = ({ tweet, mode = 'dark' }: Props) => {
  ...
  const retweetCount = formatNumber(tweet.retweets || 0)
  const viewCount = formatNumber(tweet.views || 0)
  ...
  return (
    ...
    >
      <div className={s.replyIconWrapper}>
        ...
        <span className={s.replyText}>{commentCount}</span>
      </div>
      <div className={s.retweetIconWrapper}>
        <svg viewBox="0 0 24 24" className={s.retweetIcon} aria-hidden="true">
          <g>
            <path d="M4.5 3.88l4.432 4.14-1.364 1.46L5.5 7.55V16c0 1.1.896 2 2 2H13v2H7.5c-2.209 0-4-1.79-4-4V7.55L1.432 9.48.068 8.02 4.5 3.88zM16.5 6H11V4h5.5c2.209 0 4 1.79 4 4v8.45l2.068-1.93 1.364 1.46-4.432 4.14-4.432-4.14 1.364-1.46 2.068 1.93V8c0-1.1-.896-2-2-2z"></path>
          </g>
        </svg>
        <span className={s.retweetCount}>{retweetCount}</span>
      </div>
      ...
      <div className={s.viewIconWrapper}>
        ...
        <span className={s.viewCount}>{viewCount}</span>
      </div>

      <div className="ml-auto flex items-center gap-2">
        <div className={s.bookmarkIconWrapper}>
          <svg
            viewBox="0 0 24 24"
        ...
        <div className={s.shareIconWrapper}>
          <svg viewBox="0 0 24 24" className={s.shareIcon} aria-hidden="true">
            <g>
