  ...
  try {
    if (error) {
      ...
      if (!access_token || !refresh_token || !open_id) {
        logger.error('Missing essential TikTok details', {
          access_token,
          refresh_token,
          open_id
        })
      ...
      await db.transaction(async (tx) => {
        ...
        if (userUsageData.length === 0) {
          logger.warn('User does not have usage data', { userId: user.id })
          canConnect = false
          return
        }
      ...
      if (!canConnect) {
        logger.warn('User cannot connect account', { userId: user.id })
        return NextResponse.redirect(
      ...
      if (isAlreadySaved) {
        logger.info('TikTok account already saved', {
          openId: open_id,
          userId: user.id
        })
      ...
      try {
        ...
        logger.info('Successfully saved TikTok account', {
          openId: open_id,
          userId: user.id
        })
      } catch (err) {
...
const checkIfTiktokChannelIsAlreadySaved = async ({
  ...
  try {
    const response = await db
      ...
      .from(tiktokAccounts)
      .where(
        and(eq(tiktokAccounts.userId, userId), eq(tiktokAccounts.id, openId))
      )
