...
export default function TranscribeControls() {
  ...
  const handleSplitScreenTranscribeClick = async () => {
    ...
    const [transcriptionId, transcriptionIdErr] = await getTranscriptionId(
      videoUrl
    );
          ...
          {chunks.map((chunk, index) => (
            <li key={index} className="chunk flex items-center mb-2">
              <Input
                ...
                onChange={(e) =>
                  handleTranscriptionChange(index, "start", e.target.value)
                }
                className="mr-2 w-16"
              />
              <Input
                ...
                onChange={(e) =>
                  handleTranscriptionChange(index, "end", e.target.value)
                }
                className="mr-2 w-16"
              />
              <Textarea
  ...
  return (
    <>
      <Button
        ...
        onClick={handleSplitScreenTranscribeClick}
      >
