  ...
  .handler(async ({ input }) => {
    ...
    try {
      ...
      const requiredCredits = Math.ceil(
        duration / CREDIT_CONVERSIONS.TRANSCRIBE_SECONDS
      )
      ...
      if (response.status === 500) {
        ...
        logger.error('Error initiating transcription', {
          error: text,
          videoUrl: input
        })
        await logger.flush()
...
export const getTranscription = createServerAction()
  ...
  .handler(async ({ input }) => {
    const requiredCredits = Math.ceil(
      input.duration / CREDIT_CONVERSIONS.TRANSCRIBE_SECONDS
    )
    const { user } = await getUser()
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        try {
          ...
          throw new ZSAError(
            'OUTPUT_PARSE_ERROR',
            'Invalid JSON response from Whisper'
          )
        }
        ...
        if (result.success) {
          ...
          await refundCredits(user.id, requiredCredits)
          logger.error('Invalid response from Whisper', {
            callId: input.callId,
            data
          })
          await logger.flush()
          throw new ZSAError(
            'OUTPUT_PARSE_ERROR',
            'Invalid response returned from Whisper'
          )
        }
      }
